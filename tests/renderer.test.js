...
describe('mustache render with no data', () => {
  ...
  it('should throw an error when rendering template', async () => {
    try {
      ...
      assert.deepEqual(error.message, '[Zaqar renderer error - Mustache]: Invalid template! Template should be a "string" but "function" was given as the first argument for mustache#render(template, view, partials)')
